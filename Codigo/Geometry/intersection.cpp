...
template<typename T>
bool on_line(Point<T> P, Line<T> line) { return cross(P - line.A, line.d) == T(0); }
template<typename T>
bool on_segment(Point<T> P, Line<T> seg) {  return on_line(P, seg) && dot(seg.A - P, seg.B() - P) <= T(0); }
template<typename T>
bool on_ray(Point<T> P, Line<T> ray) { return on_line(P, ray) && dot(P - ray.A, ray.d) >= T(0); }
template<typename T = double>
T point_line_distance(Point<T> P, Line<T> line) { return abs(cross(line.d, P - line.A)) / norm(line.d); }
template<typename T = double>
...
template<typename T = double>
Point<T> line_projection(Point<T> P, Line<T> line) { return line.A + proj(P - line.A, line.d); }
template<typename T>
bool collinear(Line<T> line1, Line<T> line2) { return cross(line1.d, line2.d) == T(0); }
template<typename T>
bool same_line(Line<T> line1, Line<T> line2) { return collinear(line1, line2) && cross(line1.A - line2.A, line1.d) == T(0); }
template<typename T = double>
T intersection_time(Line<T> line1, Line<T> line2) { return cross(line2.A - line1.A, line2.d) / cross(line1.d, line2.d); }
template<typename T = double>
Point<T> line_intersection(Line<T> line1, Line<T> line2) { return line1.A + line1.d * intersection_time(line1, line2); }
